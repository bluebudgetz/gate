...
func NewTransactionProcessor(redisClient *redis.Client, cfg Config) (Consumer, error) {
	return newConsumer(
		redisClient,
		processTransactions,
...
func processTransactions(m *pubsub.Message) error {
	if contentTypeValue, ok := m.Attributes["content-type"]; !ok {
		return errors.New("message is missing 'content-type' header").AddTag("msgID", m.ID)
	} else if contentType, _, err := mime.ParseMediaType(contentTypeValue); err != nil {
		return errors.New("bad 'content-type' header encountered").AddTag("msgID", m.ID)
	} else if contentType == "application/vnd.ms-excel" {
		// TODO: implement support for *.xls transactions files
		return errors.New("opening *.xls is not yet implemented").AddTag("msgID", m.ID)
	} else if contentType == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" {
		// TODO: implement support for *.xlsx transactions files
		return errors.New("opening *.xlsx is not yet implemented").AddTag("msgID", m.ID)
	} else {
